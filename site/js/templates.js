angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('pieces/team_section.html',
    "<section data-id={{section.id}}><div class=imgWrapper><img ng-src={{section.featured_image}}> <img ng-src={{section.funny_image}}><ul class=socialButtons><li class=\"anchorWrapper invert\" ng-repeat=\"social in section.content.social_media\" ng-class=\"{\n" +
  ...
  $templateCache.put('partials/project.html',
    "<nav id=projectNav><ul><li ng-click=viewProject(nextProject.id)><div class=navTitle><h5>{{nextProject.content.client}}</h5></div></li><li ng-click=viewProject(prevProject.id)><div class=navTitle><h5>{{prevProject.content.client}}</h5></div></li></ul></nav><div id=projectLayout ng-class=\"{caseStudy: project.content.case_study}\"><div class=featuredImage><img ng-src={{project.content.featured_image.url}} alt={{project.content.featured_image.alt}}></div><div class=projectContent><h1 ng-bind-html=project.title ng-if=project.content.case_study></h1><div class=projectDetails><div class=projectInfo><section><h2>Client</h2><p>{{project.content.client}}</p></section><section ng-if=project.content.project><h2>Project</h2><p>{{project.content.project}}</p></section><section ng-if=project.content.services><h2>Services</h2><p>{{project.content.services}}</p></section><section ng-repeat=\"detail in project.content.other_details\"><h2>{{detail.title}}</h2><p>{{detail.details}}</p></section></div><a class=clientSite href={{project.content.url}}>Visit Website</a><div class=socialAccounts ng-if=project.content.social_media.length><ul class=socialButtons><li ng-repeat=\"social in project.content.social_media\" ng-class=\"{\n" +
    "\t\t\t\t\t\t\tfb_btn: social.account.toLowerCase() == 'facebook',\n" +
    "\t\t\t\t\t\t\ttw_btn: social.account.toLowerCase() == 'twitter',\n" +
    "\t\t\t\t\t\t\tbe_btn: social.account.toLowerCase() == 'behance',\n" +
    "\t\t\t\t\t\t\tpi_btn: social.account.toLowerCase() == 'pinterest',\n" +
    ...
    "\t\t\t\t\t\t\tot_btn: social.account.toLowerCase() == 'other',\n" +
    "\t\t\t\t\t\t\tma_btn: social.account.toLowerCase() == 'mail'\n" +
    "\t\t\t\t\t\t}\"><a href={{social.url}}></a></li></ul></div></div><div class=projectBody ng-bind-html=project.body ng-if=project.content.case_study></div></div></div><div id=projectImages ng-if=imageSections.length><section ng-repeat=\"section in imageSections\"><h6>{{section.label}}</h6><div class=imageWrapper><div ng-repeat=\"image in section.images\" class=imageBox ng-class=\"{halfImage: section.images.length > 1}\"><img ng-src={{image.url}} alt={{image.alt}}></div></div></section></div><div id=readAbout ng-if=\"project.content.case_study && project.content.read_about.length\"><h3>Read More About {{project.content.client}}</h3><section ng-repeat=\"story in project.content.read_about\"><a href={{story.url}} target=_blank><p>{{story.title}}</p><h4>{{story.source}}</h4></a></section></div><div id=relatedProjects ng-if=\"project.content.case_study && project.content.related_projects.length\"><h3>Related Projects</h3><section ng-repeat=\"related in project.content.related_projects\" ng-click=viewProject(related.id)><div class=imgWrapper><img ng-src={{related.image.url}} alt={{related.image.alt}}></div><div class=contentWrapper><p>{{related.title}}</p><h4>{{related.client}}</h4></div></section></div>"
  );
